...
(() => {
  ...
  const textClasses = [
    'specimen','meta','tile-title','tile-desc','tile-num','tile-eyebrow',
    'swatch-name','swatch-hex','world-eyebrow','world-title','world-sub',
    'label','stat','desc','accent','cta',
    'otp-title','otp-link','resource-title','resource-desc',
    'prevnext-eyebrow','prevnext-title','page-eyebrow','page-sub',
  ...
  document.querySelectorAll('.grt-align button').forEach(b => b.addEventListener('click', () => {
  ...
  document.addEventListener('contextmenu', e => {
    ...
    const w = ctx.offsetWidth || 160, h = ctx.offsetHeight || 70;
    ctx.style.left = Math.min(e.clientX, vw - w - pad) + 'px';
    ctx.style.top  = Math.min(e.clientY, vh - h - pad) + 'px';
  });
  document.addEventListener('click', e => { if (!ctx.contains(e.target)) ctx.classList.remove('open'); });
