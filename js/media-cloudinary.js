...
(() => {
  ...
  const DB = 'praia-videos', STORE = 'blobs';
  ...
  const cldVideoSrc = (publicId, version) =>
    `https://res.cloudinary.com/${CLOUDINARY.cloud}/video/upload/w_1600,c_limit,vc_h264,q_auto,f_mp4/v${version}/${publicId}.mp4`;
  const cldPoster = (publicId, version) =>
    `https://res.cloudinary.com/${CLOUDINARY.cloud}/video/upload/w_640,c_limit,q_auto,f_auto/v${version}/${publicId}.jpg`;
  ...
  const cldImageSrc = (publicId, version) =>
    `https://res.cloudinary.com/${CLOUDINARY.cloud}/image/upload/w_1600,c_limit,q_auto,f_auto/v${version}/${publicId}`;
  const cldImageThumb = (publicId, version) =>
    `https://res.cloudinary.com/${CLOUDINARY.cloud}/image/upload/w_400,c_limit,q_auto,f_auto/v${version}/${publicId}`;
  ...
  const selectedVideoMod = () =>
    document.querySelector('.guide-page.active .video-mod.canvas-selected') ||
    document.querySelector('.video-mod.canvas-selected');
  ...
  const handlePickedVideo = async (mod, file) => {
    ...
    if (cldReady()) {
      try {
        ...
        const v = mod.querySelector('video'); if (v) v.removeAttribute('poster');
    ...
    try { await idbPut(id, file); } catch (e) { console.warn('Video persist failed', e); }
  ...
  const renderVideoGallery = async () => {
    ...
    const ratioLabel = (w, h) => {
      ...
      for (const k of known) { const d = Math.abs(k[0] - r); if (d < bd) { bd = d; best = k; } }
    ...
    const applyToMod = (src) => {
      const mod = selectedVideoMod(); if (!mod || !src) return;
      delete mod.dataset.vidId;
      const v = mod.querySelector('video'); if (v) v.removeAttribute('poster');
    ...
    const clearMod = () => {
      const mod = selectedVideoMod(); if (!mod) return;
      delete mod.dataset.vidId;
      const v = mod.querySelector('video');
      if (v) { try { v.pause(); } catch {} v.removeAttribute('poster'); v.removeAttribute('src'); try { v.load(); } catch {} }
      window.__praiaAutosave?.();
    };
    const wireTiles = () => {
      host.querySelectorAll('.grv-gtile[data-src]').forEach(b => b.addEventListener('click', () => applyToMod(b.dataset.src)));
      host.querySelectorAll('.grv-rep[data-src]').forEach(b => b.addEventListener('click', e => { e.stopPropagation(); applyToMod(b.dataset.src); }));
      host.querySelectorAll('.grv-del').forEach(b => b.addEventListener('click', e => { e.stopPropagation(); clearMod(); }));
    };
    if (!cldReady()) {
    ...
    if (__galleryBusy) return; __galleryBusy = true;
  ...
  window.__praiaReplaceVideo = (mod) => {
    ...
    let done = false;
    const cleanup = () => { if (!input.isConnected) return; setTimeout(() => input.remove(), 0); };
...
(() => {
  ...
  window.__praiaReplacePhoto = (img) => { activeImg = img || activeImg; if (activeImg) fileInput.click(); };
  ...
  window.__praiaClearPhoto = (img) => {
    const t = img || activeImg; if (!t) return;
  ...
  const urlField = document.getElementById('grp-url');
  const applyUrl = () => { const v = urlField.value.trim(); if (v && activeImg) setPhotoBg(activeImg, v); };
  urlField?.addEventListener('change', applyUrl);
  urlField?.addEventListener('keydown', e => { e.stopPropagation(); if (e.key === 'Enter') { e.preventDefault(); applyUrl(); } });
...
(() => {
  ...
  const renderPhotoGallery = async () => {
    ...
    if (!host) return;
    const cld = window.__praiaCld; if (!cld) return;
    ...
    const ratioLabel = (w, h) => {
      if (!w || !h) return '';
      const r = w / h;
      const known = [[16/9,'16:9'],[9/16,'9:16'],[1,'1:1'],[4/3,'4:3'],[3/4,'3:4'],[4/5,'4:5'],[3/2,'3:2'],[2/3,'2:3'],[21/9,'21:9']];
      let best = known[0], bd = Infinity;
      for (const k of known) { const d = Math.abs(k[0] - r); if (d < bd) { bd = d; best = k; } }
      return bd < 0.06 ? best[1] : `${w}×${h}`;
    };
    ...
    const applyToImg = (src) => { const img = window.__praiaActivePhoto(); if (!img || !src) return; window.__praiaSetPhotoBg(img, src); };
    const wireTiles = () => {
      host.querySelectorAll('.grv-gtile[data-src]').forEach(b => b.addEventListener('click', () => applyToImg(b.dataset.src)));
      host.querySelectorAll('.grv-rep[data-src]').forEach(b => b.addEventListener('click', e => { e.stopPropagation(); applyToImg(b.dataset.src); }));
      host.querySelectorAll('.grv-del').forEach(b => b.addEventListener('click', e => { e.stopPropagation(); window.__praiaClearPhoto?.(window.__praiaActivePhoto()); }));
    };
    if (!cld.ready()) {
